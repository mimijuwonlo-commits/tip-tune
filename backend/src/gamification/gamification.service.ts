...
export class GamificationService implements OnModuleInit {
  ...
  private async seedBadges() {
    const badges = [
      ...
      },
      {
        ...
        criteria: { tipsSent: 100 },
      },
      {
        name: "Early Supporter",
        category: BadgeCategory.TIPPER,
        tier: BadgeTier.SILVER,
        description: "One of the first 10 tippers of an artist.",
      ...
      },
      {
        ...
        category: BadgeCategory.TIPPER,
        tier: BadgeTier.GOLD,
        description: "Tipped in 5+ genres.",
        criteria: { genres: 5 },
      },

      // Artist Badges
      {
        name: "First Track Upload",
        category: BadgeCategory.ARTIST,
        tier: BadgeTier.BRONZE,
        description: "Uploaded your first track.",
        criteria: { tracksUploaded: 1 },
      },
      {
        name: "100 Plays",
        category: BadgeCategory.ARTIST,
      ...
      },
      {
        ...
        criteria: { totalPlays: 1000 },
      },
      {
        name: "First Tip Received",
        category: BadgeCategory.ARTIST,
        tier: BadgeTier.BRONZE,
        description: "Received your first tip.",
        criteria: { tipsReceived: 1 },
      },
      {
        name: "100 Tips Received",
        category: BadgeCategory.ARTIST,
        tier: BadgeTier.SILVER,
        description: "Received 100 tips.",
        criteria: { tipsReceived: 100 },
      },
      {
        name: "Rising Star",
        category: BadgeCategory.ARTIST,
  ...
  async handleTipVerified(payload: TipVerifiedEvent) {
    ...
    const { fromUser, artistId } = payload;

    // 1. Tipper Badges
    const user = await this.userRepo.findOne({ where: { id: fromUser } });
    if (user) {
      ...
      const genreStats = await this.tipRepo
        .createQueryBuilder("tip")
        .leftJoin("tip.artist", "artist")
        .select("COUNT(DISTINCT artist.genre)", "count")
        .where("tip.fromUser = :fromUser", { fromUser: user.id })
      ...
      for (const t of artistTips) {
        if (!uniqueTippers.has(t.fromUser)) {
          uniqueTippers.add(t.fromUser);
          rank++;
          if (t.fromUser === user.id && rank <= 10) {
            isEarly = true;
            break;
          }
        }
      ...
      if (isEarly) {
        await this.awardBadge(fromUser, "Early Supporter");
      }
    }
    ...
    if (tipsReceived >= 1)
      await this.awardBadge(artistId, "First Tip Received");
    if (tipsReceived >= 100)
      await this.awardBadge(artistId, "100 Tips Received");
  }
  ...
  async handleTrackUploaded(payload: TrackUploadedEvent) {
    ...
    if (trackCount >= 1)
      await this.awardBadge(artist.user.id, "First Track Upload");
  }
  ...
  async handleTrackPlayed(payload: TrackPlayedEvent) {
    ...
    if (totalPlays >= 1000)
      await this.awardBadge(artist.user.id, "1,000 Plays");
  }
